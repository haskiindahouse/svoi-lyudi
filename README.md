...
Хочется увидеть на карте **себя** и посчитать свои рукопожатия до героев? Скопируй
`personal.json.example` → `personal.json`, впиши себя и свои связи (вуз, работа, знакомства).
Этот файл игнорируется гитом и подмешивается **только в локальную сборку** `dist/index.html` —
публичная витрина и данные остаются картой опубликованных фактов.
