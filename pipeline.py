...
def stage_build():
    ...
    for q, p in persons.items():
        ...
        for prop, etype in PG_PROPS.items():
            for tq, y1, y2 in p["props"].get(prop, []):
                ...
                hi = y2 or min(a2, o["dissolved"] or 2020, 2020)
                if lo > hi:
                    lo, hi = min(lo, hi), max(lo, hi)
    ...
    rng = random.Random(7)
    ax = {q: min(max(anchor_year(n), 1780), 2020) for q, n in nodes.items()}
    ...
    (DIST / "artifact.html").write_text(
        "<title>Свои люди — граф русской элиты 1800–2020</title>\n" + body)  # body-only: Artifact сам оборачивает
    full = ('<!doctype html><html lang="ru"><head><meta charset="utf-8">'
            '<meta name="viewport" content="width=device-width, initial-scale=1">'
            "<title>Свои люди — граф русской элиты 1800–2020</title></head><body>"
            + body + "</body></html>")
